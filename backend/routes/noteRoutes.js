 ...
 * get all notes
 */
noteRouter.get('/get-all', async (req, res) => {
    try {
...
noteRouter.post('/',async (req,res) => {
    ...
    try {
        ...
        res.status(500).json({message:"Error creating Note", details:err.message});
    }
...
noteRouter.get('/:id',idValidator,async (req,res) => {
    ...
    try {
        ...
        res.status(500).json({message:"Error fetching note", details: err.message});
    }
})
...
noteRouter.put('/:id',idValidator ,async(req,res) => {
    ...
    try {
        ...
        res.status(200).json({message: "Note updated succesfully", note: updatedNote})
    } catch (err) {
        res.status(500).json({message:"Error updating note", details: err.message});
    }
})
...
noteRouter.delete('/:id', idValidator,async(req,res) => {
    ...
    try{
        ...
        res.status(500).json({message:"Error deleting note", details: err.message});
    }
})
